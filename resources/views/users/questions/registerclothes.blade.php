...
@section('content')
        ...
        <form method="POST" action="{{route('add') }}"　enctype="multipart/form-data">
            @csrf
             ...
             <label>画像選択<input type="file" name="clothes[img]" accept=".png,.jpg,.jpeg,image/png,image/jpg"><span class="badge badge-danger ml-2">{{ __('必須') }}</span></label>
            <label>服の名称<input type="text" name="clothes[name]" placeholder="○○パーカー"><span class="badge badge-danger ml-2">{{ __('必須') }}</span></label>
